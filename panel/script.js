...
let loading, error, errorMessage, logsSection, idsSection, mapSection, breadcrumb;
...
function initializeDOMElements() {
    ...
    if (!loading || !error || !errorMessage || !logsSection || !idsSection || !mapSection || !breadcrumb) {
...
function displayLogs(logs) {
    ...
    if (logs.length === 0) {
        tbody.innerHTML = '<tr><td colspan="2" class="text-center">No logs available</td></tr>';
        return;
    }
...
function displayIds(ids) {
    ...
    if (ids.length === 0) {
        tbody.innerHTML = '<tr><td colspan="2" class="text-center">No participants found</td></tr>';
        return;
    }
    ...
    container.innerHTML = `
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                ...
                <div class="info-value">${data.maps ? data.maps.length : 0}</div>
            </div>
        </div>
    `;
}
    ...
    locations.forEach((location, index) => {
        ...
        marker.bindPopup(`
            ...
            <div class="popup-info">
                ...
                <strong>Status:</strong> ${isFirst ? 'First Location' : isLast ? 'Latest Location' : 'Tracking Point'}
            </div>
        `);
...
function displayLocationStats(locations) {
    ...
    const timeSpan = new Date(lastLocation.timestamp) - new Date(firstLocation.timestamp);
    ...
    container.innerHTML = `
        ...
        <div class="stats-grid">
            ...
            <div class="stat-item">
                <div class="stat-value">${formatDateTime(firstLocation.timestamp)}</div>
                <div class="stat-label">First Seen</div>
            </div>
            <div class="stat-item">
                <div class="stat-value">${formatDateTime(lastLocation.timestamp)}</div>
